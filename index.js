...
class new_person_card_class extends PIXI.Container{
	...
	fill_data(person_data, type){
		...
		if (type==='spouse'){
			...
			if (parents[0]&&!familyData[parents[0]].empty)	have_parents=1
			if (parents[1]&&!familyData[parents[1]].empty)	have_parents=1
			
			if (person_data.spouses.length>1||have_parents){				
...
rel={
	...
	calc_cards_height(){
		...
		for (const card of objects.rel_cards_pool){
			if(!card.visible) continue
			tot_h=Math.max(tot_h,card.y+card.height)
			cards_num++
		}
	...
	update(){
		...
		objects.rel_tree_cont.y=objects.rel_tree_cont.sy
		objects.rel_tree_cont.x=objects.rel_tree_cont.sx
	...
	down(e){
		...
		this.drag_current.x=mx
		this.drag_current.y=my
	...
	move(e){
		...
		const d_vec = {
			x:this.drag_current.x-this.drag_prev.x,
			y:this.drag_current.y-this.drag_prev.y
		}

		this.drag_prev.x=this.drag_current.x
...
add_dlg={
	...
	async remove_person_down(){
		...
		const resp=await conf_dlg.show('Уверены что хотите удалить данную персону?')
		if (!resp) return
...
dp={
	...
	btns_data:[
		...
		[30,719,90,769,"year_switch",-10],
		[360.2,719,420.2,769,"year_switch",10],
	...
	set_day(d,recheck){
		...
		const days_in_month=new Date(this.cur_year, this.cur_month, 0).getDate()
		if (d>days_in_month)
			return
		...
		const btn_data=this.btns_data.find(d=>{return d[4]==='day'&&d[5]===this.cur_day})		
		objects.dp_cur_day.x=btn_data[0]-10
		objects.dp_cur_day.y=btn_data[1]-10
		
		if (!recheck) this.update_date()
	...
	recheck_day(){
		...
		//где поставить оверлей
		const g_pos={28:41,29:95,30:150,31:204}[days_in_month]
		objects.dp_days_overlay.x=g_pos-3
		
		//исправляем если день некорректный
		if (this.cur_day>days_in_month){
			this.cur_day=days_in_month
			this.set_day(this.cur_day,1)
		}
	...
	update_month_view(){
		...
		this.cur_month_layer_id=Math.floor((this.cur_month-1)/4)
		this.cur_month_cell_id=(this.cur_month-1)%4
		objects.dp_month.texture=assets.date_pick_months_pack[this.cur_month_layer_id]		
		
		//подсвечиваем текущий месяц
		const btn_data=this.btns_data.find(d=>{return d[4]==='month'&&d[5]===this.cur_month_cell_id})
	...
	switch_month_layer(d){
		...
		const m=this.cur_month_layer_id*4+this.cur_month_cell_id+1
...
conf_dlg={
	...
	show(info){
		...
		anim3.add(objects.conf_dlg_cont,{alpha:[0,1,'linear']}, true, 0.25)
		objects.conf_dlg_info.text=info
		return new Promise(res=>{
			this.resolver=res
		})
	},
	...
	down(e){
		...
		if (my<440||my>520) return	
		
		if (mx<220) this.resolver(0)
		if (mx>230) this.resolver(1)
	...
	close(){		
	en_keys:[[30,120,54.5,159.07,'1'],[65.38,120,89.88,159.07,'2'],[100.76,120,125.26,159.07,'3'],[136.14,120,160.64,159.07,'4'],[171.52,120,196.02,159.07,'5'],[206.89,120,231.39,159.07,'6'],[242.27,120,266.77,159.07,'7'],[277.65,120,302.15,159.07,'8'],[313.03,120,337.53,159.07,'9'],[348.41,120,372.91,159.07,'0'],[419.17,120.93,460,160,'<'],[81.35,170.88,105.85,209.95,'Q'],[115.13,170.88,139.63,209.95,'W'],[148.91,170.88,173.41,209.95,'E'],[182.68,170.88,207.18,209.95,'R'],[216.46,170.88,240.96,209.95,'T'],[250.24,170.88,274.74,209.95,'Y'],[284.02,170.88,308.52,209.95,'U'],[317.79,170.88,342.29,209.95,'I'],[351.57,170.88,376.07,209.95,'O'],[385.35,170.88,409.85,209.95,'P'],[108.8,219.72,133.3,258.79,'A'],[142.58,219.72,167.08,258.79,'S'],[176.36,219.72,200.86,258.79,'D'],[210.14,219.72,234.64,258.79,'F'],[243.92,219.72,268.42,258.79,'G'],[277.71,219.72,302.21,258.79,'H'],[311.49,219.72,335.99,258.79,'J'],[345.27,219.72,369.77,258.79,'K'],[379.05,219.72,403.55,258.79,'L'],[27.43,269.06,65.98,308.13,'*'],[101.26,269.06,125.76,308.13,'Z'],[134.18,269.06,158.68,308.13,'X'],[167.1,269.06,191.6,308.13,'C'],[200.01,269.06,224.51,308.13,'V'],[232.93,269.06,257.43,308.13,'B'],[265.84,269.06,290.34,308.13,'N'],[298.76,269.06,323.26,308.13,'M'],[364.59,269.06,389.09,308.13,')'],[383.79,120,408.29,159.07,'?'],[23.68,317.4,146.18,356.47,'CLOSE'],[154.35,317.4,342.18,356.47,' '],[350.35,317.4,464.68,356.47,'SEND'],[412.83,219.72,437.33,258.79,','],[331.67,269.06,356.17,308.13,'('],[27.43,219.72,65.98,258.79,'RU'],[397.5,269.06,422,308.13,'.']],
	layout:0,
	resolver:0,
	layout_case:0,
	prv_key:'1',
	...
	switch_case(tar_case){
		...
		if (this.layout===this.ru_keys){
			...
			objects.chat_keyboard.texture=[assets.eng_layout_up,assets.eng_layout_low][this.layout_case];
		}
		need_render=1
	},
	...
	keydown (key) {
		...
		const key2 = this.layout.find(k => {return k[4] === key})
...
main_loader={
	...
	async load2(){
		...
		loader.add('rus_layout_up', git_src+'res/keyboards/rus_layout_up.png')
		loader.add('rus_layout_low', git_src+'res/keyboards/rus_layout_low.png')
		loader.add('eng_layout_up', git_src+'res/keyboards/eng_layout_up.png')
		loader.add('eng_layout_low', git_src+'res/keyboards/eng_layout_low.png')
